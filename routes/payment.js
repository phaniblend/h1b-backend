...
router.post('/process', (req, res) => {
    ...
    setTimeout(() => {
        ...
        newTransaction.receiptUrl = `/receipts/payment-${newTransaction.id}.pdf`;
    }, 2000);
    res.json({
        success: true,
        data: newTransaction,
...
router.get('/savings', (req, res) => {
    ...
    if (salary) {
        const annualSalary = parseInt(salary);
        const cutPercentage = employerCut ? parseFloat(employerCut) / 100 : 0.25;
